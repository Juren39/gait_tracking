...
def run(args):
    ...
    output_path = '../yolov8/output/tracked_video.mp4'  # 输出文件路径
...
def draw_tracking_results(image, tracks):
    ...
    Args:
        image (np.ndarray): 当前图像帧。
        tracks (list): 跟踪器输出的跟踪结果，每个跟踪结果通常包含 [x1, y1, x2, y2, track_id]。

    """
    for track in tracks:
        # 提取跟踪信息
        x1, y1, x2, y2, track_id = track[:5]  # 获取坐标和跟踪 ID
